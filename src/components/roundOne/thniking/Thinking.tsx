...
//           {player.thinking ? (
//             <p className="text-zinc-500 text-sm italic flex items-center justify-center gap-1">
    ...
    <div className="min-h-screen w-full flex flex-col items-center">
      ...
      <div className="w-full max-w-7xl px-4 sm:px-6 pb-5 flex flex-wrap justify-center gap-3">
        {[
          { label: "Round", value: "1 of 5" },
          { label: "Points", value: "100 pts" },
          { label: "Answered", value: `${players.length} Players` },
        ].map((s) => (
          <div
            key={s.label}
            className="flex items-center gap-2 px-4 py-1.5 rounded-full border border-white/8 bg-white/4 backdrop-blur-sm"
      ...
      {/* <main className="w-full max-w-7xl px-4 mt-6 sm:px-6 lg:px-8 pb-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-[1fr_1fr_320px] xl:grid-cols-[1fr_1fr_360px] gap-4 sm:gap-5">
        {players.length === 0 ? (
          <div className="col-span-2 flex flex-col items-center justify-center min-h-[200px] gap-3">
      ...
      {/* Main layout */}
      <div className="w-full max-w-7xl px-4 sm:px-6 lg:px-8 pb-10 flex gap-5">
        {/* ── Left: Player cards grid ── */}
        <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5 content-start">
          {players.length === 0 ? (
            <div className="col-span-2 flex flex-col items-center justify-center min-h-[200px] gap-3">
              <span className="inline-flex items-center gap-1">
                {[0, 1, 2].map((i) => (
        ...
        {/* ── Right: Sidebar ── */}
        <div className="hidden lg:block w-[320px] xl:w-[360px] flex-shrink-0 self-start sticky top-5 max-h-[calc(100vh-6rem)] overflow-y-auto">
          <ParticipantPanel />
        </div>
      </div>
