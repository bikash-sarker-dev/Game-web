...
// type CallPhase = "waiting" | "incoming" | "connecting" | "active";
...
// // ─── ZegoCloud Room (participant side) ────────────────────────────────────────
...
//     // Every argument comes from the API — changing any one causes error 20014.
...
//   // roomId we send to the API — must match exactly what the host sends.
//   // Host sends: player.id (= this user's id) + GAME_ID
//   // We send:    currentUser.id              + GAME_ID  → same value ✓
//   const roomId = `${currentUser?.id ?? ""}${gameId}`;

//   // ── Fetch Zego token from your API ────────────────────────────────────────
//   // Only fetch when participant clicks Accept (connecting phase).
//   // Query sends: { userId: currentUser.id, roomId }
...
//     isError,
//   } = useZegoCloudQuery(
//     { userId: currentUser?.id ?? "", roomId },
//     { skip: callPhase !== "connecting" },
//   );

//   const zego = apiResp?.data; // { token, appId, roomId, userId, expiredAt }

//   // Transition connecting → active once all 4 fields arrive from the API
//   useEffect(() => {
//     if (
...
//       setCallPhase("active");
//     }
//   }, [callPhase, isLoading, zego]);

//   // Error fallback
//   useEffect(() => {
//     if (callPhase === "connecting" && isError) {
//       showToast("Connection failed — please try again.");
//       setTimeout(() => setCallPhase("waiting"), 3000);
//     }
//   }, [callPhase, isError]);

//   // Switch to incoming when the socket delivers INCOMING_CALL
//   useEffect(() => {
//     if (incomingHostId && callPhase === "waiting") setCallPhase("incoming");
//   }, [incomingHostId]);
...
//   // ── ACCEPT — participant taps "Join call" ─────────────────────────────────
//   const handleJoin = () => {
...
//     );
//     // Triggers the RTK Query fetch (skip flips from true → false)
...
//                 {isLoading ? "Fetching secure token…" : "Entering room…"}
//               </p>
//             </div>
//           </div>
//         )}

//         {/* ── ACTIVE: real ZegoCloud room ── */}
//         {/* Mount ONLY when all 4 API fields are confirmed — prevents 20014/50119 */}
//         {callPhase === "active" &&
//           zego?.token &&
//           zego?.appId &&
//           zego?.roomId &&
...
//                 userName={
//                   currentUser?.name ?? currentUser?.username ?? "Contestant"
//                 }
//                 onLeave={handleLeave}
//               />
//             </div>
